...
public struct PrismAdaptiveMeshBackground: View {
    ...
    public var body: some View {
        if #available(macOS 15.0, *) {
            ...
            PrismBackgroundWash {
                GeometryReader { geometry in
                    PrismAdaptiveLayeredTransitionField(
                        sourceColor: sourceColor,
                        palette: palette,
                        transitionStart: transitionStart,
                        radiusBasis: max(geometry.size.width, geometry.size.height)
...
private struct PrismAdaptiveLayeredTransitionField: View {
    let sourceColor: Color
    let palette: PrismAdaptiveBackgroundPalette
    let transitionStart: CGFloat
    ...
    var body: some View {
        ...
        let middle = start + (1 - start) * PrismBackgroundConstruction.middleStopLocation

        Rectangle()
            .fill(
                LinearGradient(
                    stops: [
                        .init(color: sourceColor, location: 0),
                        .init(color: sourceColor, location: start),
                        .init(color: palette.middleColor, location: middle),
            ...
            .overlay {
                RadialGradient(
                    colors: [
                        palette.glowColor.opacity(PrismBackgroundConstruction.glowOpacity),
                        .clear,
                    ],
